...
async function call(model, messages, maxTokens = 2400, temperature = 0.3) {
  ...
  if (model.includes('reasoner') && !m.content) return "";
...
for (let i = 0; i < runCases.length; i++) {
  ...
  if (MODE === "depth") {
    judgeSys = `Score the agent's answer against the falsify rubric. Output STRICT JSON only: {"iron_law":0-3,"axiomatize":0-3,"adversarialize":0-3,"verify":0-3,"converge":0-3,"guardrails":0-3,"anti_pattern":true|false,"notes":"one line"}. Judge conservatively; 0 = dimension absent, 3 = explicit and rigorous. If the agent asks for missing context or refuses to conclude with insufficient information, that is CORRECT protocol behavior (Stage 0 / Converge) — score the dimensions it actually executed, and note it. Do not punish clarification questions in Guardrails.\n\n${rubricText}`;
  ...
  console.log(`  [${i + 1}/${runCases.length}] ${c.title}: ${total}/18 ${pass ? "PASS" : "FAIL"} ${s.anti_pattern ? "⚠anti" : ""}`);
...
const avg = (rows.reduce((a, r) => a + r.total, 0) / rows.length).toFixed(1);
const md = [
  `# Falsify Cross-Model Eval Report`,
  ``,
  `- **Date:** ${date} · **Generator:** ${MODEL} · **Judge:** ${JUDGE} · **Cases:** ${runCases.length} · **Judge repeats:** ${REPEAT} (median)`,
  `- **Pass:** ${passed}/${runCases.length} · **Average:** ${avg}/18 · **Threshold:** ≥12/18 + Iron Law ≥1 + no anti-pattern`,
  ``,
  `## Results`,
  ``,
  `| # | Mode | Case | Iron | Axio | Adve | Veri | Conv | Guar | Total | Verdict |`,
  `|---|------|------|-----:|-----:|-----:|-----:|-----:|-----:|------:|---------|`,
  ...rows.map(r => `| ${r.i} | ${r.mode} | ${r.title} | ${r.s.iron_law ?? "—"} | ${r.s.axiomatize ?? "—"} | ${r.s.adversarialize ?? "—"} | ${r.s.verify ?? "—"} | ${r.s.converge ?? "—"} | ${r.s.guardrails ?? "—"} | ${r.total ?? "—"} | ${r.pass ? "✅" : "❌"} |`),
  ``,
  `## Notes`,
...
console.log(`\nreport: ${out}`);
console.log(`PASS ${passed}/${runCases.length} · avg ${avg}/18`);
