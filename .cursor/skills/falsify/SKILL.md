...
When the question does not warrant full depth but the answer will still be acted on, do not run the five stages — append **at most 2–3 short questions**, once per conversation, each tied to something specific in the answer just given. **This includes low-stakes rough-estimate / ballpark asks** ("rough estimate", "about how much", "大概") — those are nudge territory by default, not depth. **High-stakes ballparks are NOT nudge territory:** a rough medication dose, security capacity, or production sizing estimate is Depth — being wrong there costs more than the shortcut saves, so the agent must gather key inputs and falsify before committing to a number.
